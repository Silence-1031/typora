...
多源 BFS 是 **普通 BFS 的扩展**，核心是**同时以多个点为起点**进行广度优先遍历，适用于「求所有点到**最近的起点**的最短距离」的场景（比如本题 “用户到最近店铺的距离”）。

### 一、多源 BFS 的核心特点

对比普通 BFS，多源 BFS 的关键差异在于**起点数量**和**初始化方式**：

| 特性       | 普通 BFS                 | 多源 BFS                           |
| ---------- | ------------------------ | ---------------------------------- |
| 起点数量   | 1 个起点                 | ≥2 个起点                          |
| 初始化逻辑 | 单个起点入队，距离设为 0 | **所有起点同时入队**，距离均设为 0 |
| 遍历目标   | 单一起点到其他点的距离   | 所有点到「最近起点」的距离         |
| 时间复杂度 | O (N)（N 为节点数）      | O (N)（仅需遍历一次）              |

### 二、多源 BFS 的工作原理

BFS 的本质是**按 “距离层” 扩散**（先处理距离为 0 的点，再处理距离为 1 的点，依此类推）。多源 BFS 相当于把**所有起点的 “0 距离层” 一次性加入队列**，后续扩散过程和普通 BFS 完全一致：

1. **初始化**：将所有起点的距离设为 0，并加入队列；
2. **层序扩散**：从队列中取出一个点，遍历其相邻的未访问点，将这些点的距离设为「当前点距离 + 1」，并加入队列；
3. **终止条件**：队列为空时，所有可达点的距离已被更新为 “到最近起点的最短距离”。

### 三、多源 BFS 的适用场景

只要是需要求「多个起点中**最近的一个**到其他点的最短距离」的问题，都可以用多源 BFS 高效解决，典型场景包括：
...
3. 感染扩散问题
   ...
   - 多个感染源同时扩散，求每个位置被感染的最早时间。

### 四、多源 BFS 的代码模板（以网格为例）

以本题的 “n×n 网格，多起点求最近距离” 为例，模板如下：
...
const int MAXN = 1005;
int dist[MAXN][MAXN];   // 存储到最近起点的距离
bool blocked[MAXN][MAXN];// 障碍标记
int dx[] = {-1, 1, 0, 0};// 上下左右方向
int dy[] = {0, 0, -1, 1};

int main() {
    int n, m; // n:网格大小，m:起点数量
    cin >> n >> m;
    ...
    while (!q.empty()) {
        ...
        for (int d = 0; d < 4; ++d) {
            ...
            if (nx >= 1 && nx <= n && ny >= 1 && ny <= n 
                && !blocked[nx][ny] && dist[nx][ny] == INT_MAX) {
...
相比 “对每个起点单独做普通 BFS，再取每个点的最小距离” 的方案：

- **时间复杂度更优**：普通 BFS 做 m 次的复杂度是 O (m×N)，而多源 BFS 仅需 O (N)（N 为网格总节点数）；
- **代码更简洁**：无需多次 BFS 和距离比较，一次遍历即可完成。



### 一、tuple 是什么？

`tuple`（元组）是 C++11 及以上标准提供的**固定大小、可存储不同类型元素**的容器，定义在 `<tuple>` 头文件中。它可以看作是 “增强版的结构体”—— 无需自定义结构体，就能快速封装多个不同类型的值（比如同时存 int、double、string），常用来临时存储一组相关数据（如本题中存储客户的 x 坐标、y 坐标、订餐量）。

### 二、tuple 的核心特性

| 特性          | 说明                                                         |
| ------------- | ------------------------------------------------------------ |
| 元素类型灵活  | 可包含不同类型的元素，比如 `tuple<int, double, string>` 是合法的 |
| 大小固定      | 初始化后元素数量不可修改（区别于 vector）                    |
| 可拷贝 / 赋值 | 支持直接赋值、拷贝构造，比如 `auto t2 = t1;`                 |
| 支持比较运算  | 按元素顺序逐一比较（需元素本身支持比较），比如 `t1 == t2`    |
...
// 方式1：直接初始化（C++11及以上）
tuple<int, int, int> t1(1, 5, 1);  // 对应客户：x=1, y=5, 订餐量=1
// 方式2：用make_tuple（自动推导类型）
auto t2 = make_tuple(2, 3, 3);     // 类型为 tuple<int, int, int>
// 方式3：结构化绑定（C++17及以上，推荐）
auto [x, y, c] = t1;               // x=1, y=5, c=1
```

#### 2. 访问元素

通过 `get<索引>(tuple对象)` 访问，**索引从 0 开始**：
...
```cpp
tuple<int, string, double> t(10, "test", 3.14);
// 访问第0个元素（int）
cout << get<0>(t) << endl;  // 输出 10
// 访问第1个元素（string）
cout << get<1>(t) << endl;  // 输出 test
// 访问第2个元素（double）
cout << get<2>(t) << endl;  // 输出 3.14

// 注意：索引必须是编译期常量（不能用变量）
// 错误示例：int idx=0; get<idx>(t); 
```
...
tuple<int, int> t(1, 2);
get<1>(t) = 5;              // t 变为 (1,5)
cout << get<1>(t) << endl;  // 输出 5
```

#### 4. 解包（结构化绑定，C++17+）

这是最常用的便捷方式，直接将 tuple 的元素拆分到变量中：
...
customers.emplace_back(1,5,1);
customers.emplace_back(2,3,3);

// 结构化绑定解包
for (auto [x, y, c] : customers) {
...
| 函数                             | 作用                                                         |
| -------------------------------- | ------------------------------------------------------------ |
| `tuple_size<decltype(t)>::value` | 获取 tuple 的元素个数（编译期常量），比如 `tuple_size<decltype(t1)>::value` 输出 3 |
| `tie(a,b,c)`                     | 将变量绑定到 tuple，用于解包（兼容 C++11/14）                |
...
```cpp
tuple<int, int, int> t(1,5,1);
int x, y, c;
// 将 t 的元素赋值给 x,y,c
tie(x, y, c) = t;
cout << x << "," << y << "," << c << endl;  // 输出 1,5,1
```

### 四、tuple 在本题中的应用场景

本题中用 `vector<tuple<int, int, int>> customers` 存储客户信息，优势在于：

1. **无需自定义结构体**：省去写 `struct Customer { int x,y,c; }` 的代码，快速封装多字段数据；
2. **遍历便捷**：结合结构化绑定，一行代码即可解包出 x、y、订餐量；
3. **类型安全**：编译期检查元素类型和数量，避免手动定义结构体的笔误。
...
1. **索引是编译期常量**：`get<idx>` 中的 idx 必须是常量（比如 `get<0>`），不能用变量（比如 `int i=0; get<i>` 报错）；
2. **C++ 版本兼容**：结构化绑定需要 C++17 及以上，如果编译器版本较低，可用 `tie` 替代；
3. **元素数量不宜过多**：如果需要存储超过 3-4 个元素，建议用自定义结构体（可读性更高）。