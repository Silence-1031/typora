...
Java 采用**抢占式调度**（核心），按照优先级和CPU空闲状态抢占执行，底层依赖操作系统的线程调度（JVM 不直接控制 CPU 分配），通过`Thread`类的`setPriority(int)`/`getPriority()`方法设置 / 获取，会让优先级高的线程获得更多 CPU 时间片；优先级仅为 “调度建议”，不保证 CPU 一定优先执行高优先级线程

**线程调度核心方法**

- `static void yield()`（静态方法，当前线程调用）：让当前线程主动放弃 CPU 使用权，从 “运行态” 回到 “就绪态(Runnable)”，重新参与 CPU 竞争（注意：不是进入阻塞态，且不保证其他线程能抢到 CPU，可能自己再次被调度）。

- `void join()`（其他线程调用）：让 “调用该方法的线程” 优先执行，当前线程进入` “WAITING/TIMED_WAITING” `状态，直到目标线程执行结束（或等待超时）后，当前线程才恢复就绪态。

> 子线程默认继承创建它的父线程优先级

线程休眠与等待方法:用于控制线程的执行时机，实现 “延迟执行” 或 “线程间同步等待”。
...
> - volatile 通过在变量读写前后插入**内存屏障**（LoadLoad、StoreStore、LoadStore、StoreLoad），阻止指令重排，保证执行顺序与代码顺序一致
...
1. **互斥性**：锁资源只能被一个线程持有（如 synchronized/ReentrantLock）；
2. **持有并等待**：线程持有已获取的锁，同时等待其他线程持有的锁；
3. **不可剥夺**：线程持有的锁不能被强制剥夺，只能主动释放；
4. **循环等待**：多个线程形成锁的循环依赖（如 A→B→C→A）。

这是造成死锁必须要达到的4个条件，如果要避免死锁，只需要不满足其中某一个条件即可。而其中前3个条件是作为锁的条件，所以要避免死锁就需要打破第4个条件，不出现循环等待锁的关系。
...
利用jps（JVM Process Status Tool） + jstack

执行`jps`（JVM Process Status Tool），获取死锁进程的 PID（进程 ID）：

```bash
jps
# 输出示例：1234 DeadLockDemo （1234是PID）
```

 执行`jstack <PID>`，打印线程堆栈信息，jstack 会**主动检测并标记死锁**：

```bash
jstack 1234
```

输出中会包含 “Found one Java-level deadlock:” 段落，明确显示：
...
- 类加载器（ClassLoader）是 JVM 负责将.class 字节码文件加载到内存并生成 Class 对象的核心组件，也就是通过一个类的全类名来获取描述该类的二进制字节流。

- 主要有以下四种类加载器

1. 启动类加载器(Bootstrap ClassLoader)：用来加载java核心类库,如` java.lang.*`
2. 扩展类加载器(extension class loader)：它用来加载 Java 的扩展库。java虚拟机会提供一个扩展库目录。该类加载器在此目录里面查找并加载 Java 类
...
> 遵循 “双亲委派模型”（先委托父加载器加载，父加载失败再自行加载）,避免类重复加载，保证核心类（如`java.lang.String`）的全局唯一性

- 类加载的完整过程为**加载→链接（验证→准备→解析）→初始化**，加载阶段类加载器查找并读取.class 字节码，转化为方法区的运行时数据结构，在堆中创建`Class`对象；链接阶段验证字节码的合法性、为静态变量分配内存并设默认值、替换符号引用为直接引用；初始化阶段执行类构造器`<clinit>()`方法，仅在类首次主动使用时触发且保证线程安全（多个线程同时初始化一个类时，仅一个线程执行，其他线程阻塞等待）。

- 什么时候会使用到加载器？

  java中的加载器是按需加载，什么时候用到，什么时候加载
