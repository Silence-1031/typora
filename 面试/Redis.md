...
Redis 哨兵模式（Sentinel）是 Redis 官方提供的**高可用解决方案**，用于监控 Redis 主从架构中的节点状态，并在主节点故障时自动完成故障转移，保证服务不中断。

### 核心作用

1. **监控（Monitoring）**

	哨兵进程会持续检查主节点和从节点是否处于正常运行状态。

2. **自动故障转移（Automatic Failover）**

	- 当哨兵判定主节点宕机后，会从该主节点的所有从节点中，选举一个最优的从节点升级为新主节点；
	- 让剩余的从节点重新复制新主节点的数据；
	- 通知客户端更新主节点地址，实现无感切换。

3. **通知（Notification）**

	可以通过 API 向管理员或其他系统发送节点故障、切换完成等事件通知。

### 核心架构

- **主节点（Master）**：处理读写请求。
- **从节点（Slave）**：复制主节点数据，仅处理读请求，作为主节点的备用节点。
- **哨兵节点（Sentinel）**：独立的监控进程，通常部署 **3 个及以上**以提高判定的准确性（避免脑裂）。

### 核心特点

- 依赖 Redis 主从复制，**不支持数据分片**，无法解决单节点容量瓶颈（这是和 Redis 集群的核心区别）。
- 配置简单，无需修改 Redis 核心配置，只需单独配置哨兵进程。
- 支持自动故障转移，无需人工介入，降低运维成本。

### 一句话总结

Redis 哨兵模式是**主从架构的高可用保障方案**，通过监控主节点状态实现自动故障转移，保证服务可用性，但不解决数据分片和扩容问题。

Redis 哨兵模式核心是解决 Redis 主从架构中的高可用相关问题，弥补纯主从复制的缺陷，具体如下：

1. **主节点故障需手动切换的低效问题**：纯主从架构中主节点故障后，得人工排查故障、挑选合适从节点升级为主、配置其他从节点同步新主节点，还得修改客户端连接地址。这个过程耗时长且易出错，会造成服务长时间中断，而哨兵能自动完成故障转移，把服务中断时间大幅缩短。
2. **节点状态难实时监控的问题**：人工很难实时监测主从节点的运行状态，可能主节点宕机很久后才被发现。哨兵会每秒向所有主从节点发送心跳包，还会定期获取节点拓扑和复制进度等信息，能快速察觉节点异常，同时通过主观下线和客观下线的判定机制，减少网络抖动等造成的误判。
3. **客户端难动态获取主节点地址的问题**：主节点故障切换后，新主节点地址会变化。若客户端硬编码主节点地址，就需手动修改配置才能重新连接。哨兵可作为配置提供者，客户端能通过哨兵动态获取当前有效主节点地址，故障转移后客户端会自动更新连接信息，无需人工干预。
4. **故障与状态难及时知晓的问题**：主从节点出现异常时，运维人员难以及时获知。哨兵具备通知功能，当节点出现故障、完成故障转移等关键事件时，可通过脚本以邮件、API 调用等方式向管理员或其他系统发送通知，方便快速处理问题。
5. **监控节点自身单点故障的问题**：若仅部署一个哨兵节点，该节点故障后就无法实现监控和故障转移功能。而哨兵支持多节点部署，通常建议部署 3 个及以上奇数个哨兵节点，节点间通过协议同步状态，还会通过选举机制确定领导者执行故障转移，避免自身出现单点故障。
...
Redis 集群是 Redis 官方提供的**分布式解决方案**，用于解决单节点 Redis 的存储容量上限、性能瓶颈和单点故障问题，实现数据的分片存储与高可用。

### 核心特点

1. **数据分片**

	采用 **16384 个哈希槽**（hash slot）机制，集群中每个主节点负责一部分槽位。

	当客户端写入数据时，Redis 会对 `key` 计算哈希值，映射到对应的槽位，再由负责该槽位的主节点存储数据。

	这种分片方式避免了单节点存储容量不足的问题，且支持动态扩缩容。

2. **高可用架构**

	集群采用 **主从复制** 模式：每个主节点可以配置一个或多个从节点。

	- 主节点故障时，其从节点会通过选举自动升级为主节点，保证服务不中断；
	- 从节点还可以分担读请求，提升集群的读性能。

3. **去中心化**

	集群中所有节点地位对等，无需中心节点协调。

	节点之间通过 Gossip 协议交换集群状态信息（如节点上线 / 下线、槽位分配）。

4. **有限的一致性**

	Redis 集群保证**最终一致性**，主节点故障切换时，可能存在少量数据丢失（可通过配置 `replica-read-only` 等参数优化）。

### 核心限制

- 不支持多 `key` 操作（如 `MSET`），除非这些 `key` 映射到同一个哈希槽；
- 不支持事务的跨节点执行；
- 集群模式下，部分命令（如 `KEYS`）的执行效率会降低。

### 一句话总结

Redis 集群是通过**哈希槽分片 + 主从复制**实现的分布式存储方案，既能突破单节点的容量和性能限制，又能保证服务高可用，是大规模 Redis 部署的首选方案。
