设计模式是解决特定场景下软件设计问题的 “最佳实践”，Java 开发中常用的设计模式可按**创建型、结构型、行为型**三大类划分，以下是高频使用的模式，附核心思想、典型应用场景和极简代码示例：
...
- **核心思想**：通过复制已有实例（原型）创建新对象，避免重复初始化。
- **应用场景**：对象创建成本高（如大数据对象、连接池），`java.lang.Cloneable`接口
...
- **核心思想**：为复杂子系统提供统一入口，简化客户端调用。
- **应用场景**：MyBatis 的`SqlSession`、Spring 的`ApplicationContext`、电商系统的 “下单门面”（整合库存、支付、物流）。
...
// 使用：new Facade().doOperation();
```

### 5. 组合模式（Composite）

- **核心思想**：将对象组合成树形结构，统一处理 “单个对象” 和 “组合对象”。
- **应用场景**：树形结构（菜单 / 目录、部门组织架构）、MyBatis 的`SqlNode`。
...
- **核心思想**：提供统一方式遍历集合，隔离集合底层结构。
- **应用场景**：`java.util.Iterator`（ArrayList/HashMap 等集合的遍历）、MyBatis 的结果集遍历。
