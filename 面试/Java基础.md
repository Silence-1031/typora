...
OOP面向对象是一种以对象为核心的编程思想，将现实事务抽象成程序中的对象，并给对象定义一系列的属性和行为，通过对象之间的交互来实现业务逻辑，而非像面向过程那样按 “步骤 / 流程” 线性编写代码。这种思想的核心是 “**高内聚、低耦合**”：每个对象只负责自己的职责（内聚），对象之间通过接口交互，而非直接操作对方的内部数据（低耦合）。让代码更易维护且更易扩展。
...
多态：同一行为在不同的对象上表现出不同形态，核心是“父类引用指向子类对象”+“子类中方法重写”

**java如何实现OOP思想？**

封装：类和对象，访问控制（封装如何合理暴露）如仅类内部用的成员用 private，需子类继承用 protected，对外暴露用 public
...
2. 重载（Overload）和重写（Override）有什么区别？（4点）

重载（Overload）和重写（Override）是 Java 中实现多态的两种核心方式，区别主要在于

- 定义的位置不同：重载是**同一个类中多个方法间的关系**，方法名相同但参数列表不同；重写是子类与父类/接口与实现类之间的关系，子类重新实现父类中已有的方法
...
接口和抽象类是 Java 实现抽象的两种核心方式，但他们都不能实例化，必须重写抽象类和接口中的所有方法，区别主要在于：

- 设计定位不同

  抽象类核心是抽取一组**相关类的共性特征**，既包含**共性属性也包含共性方法**，侧重代码复用和继承关系；

  接口是一组抽象的行为规范，不关注类的共性属性，只关注类能做什么，侧重行为约束和解耦扩展，体现的是 “can-do” 的关系（比如 Runnable 接口，只要类实现它，重写接口的run方法，就具备执行线程任务的能力）。

-  核心语法规则不同

  - 成员变量：抽象类可以定义普通成员变量（比如 String name），也支持静态变量、常量；而接口中的成员变量**默认且只能是 public static final 修饰的常量**，无法定义普通变量。

  - 方法类型：抽象类可同时包含抽象方法、普通非抽象方法、静态方法；接口在 JDK8 + 后，默认方法是抽象方法，也支持定义 default 默认方法和 static 静态方法，但所有方法默认都是 public 修饰，不能用 private/protected，（实际上部分破坏了接口“纯抽象规范”的定位）。
...
- 深拷贝：还会递归复制所有引用类型属性指向的对象（即新对象的引用属性指向全新的内存地址）
...
- 引用变量本身的存储位置取决于它的 “作用域”（局部 / 成员 / 静态），而引用指向的实际对象 / 数组始终存储在**堆内存（线程共享区）**

1. 局部变量（方法内定义）：存储在**虚拟机栈**的栈帧中（如`public void test() { User u = new User(); }`中，`u`是栈中引用，`new User()`是堆中对象）；

2. 成员变量（类的非静态字段）：随对象一起存储在**堆**中（如`class A { User u; }`，`A`的实例在堆中，其`u`引用也在堆的该实例里）；

3. 静态引用变量（类的 static 字段）：存储在方法区（元空间）中（如`class A { static User u; }`，`u`作为类的静态成员，存于元空间）；

   无论哪种引用变量，其指向的实际对象（`new`出来的实例）永远在堆中，引用变量仅保存堆对象的内存地址。



7. #### ==和equals区别

- `==`：是运算符，判断的是 “内存地址是否相同”（即是否指向同一个对象）：
...
String 类**不能被继承**，核心原因是 String 类被`final`关键字修饰。String 常被用作 **HashMap、HashSet** 等集合的键，其哈希值基于字符内容计算后会缓存（`hash`成员变量）。若 String 可被继承，子类修改字符内容会导致哈希值变化，进而引发集合查找、存储异常（比如存进去的键和取出来的键哈希值不一致）；`final`修饰类杜绝了这种风险，保证哈希值的计算是稳定的。并且String 常用于存储敏感信息（如密码、网络地址），不可变性保证其内容不会被恶意篡改；本质是为了**强制保证其不可变性**，进而支撑线程安全、哈希值稳定等核心特性，同时兼顾性能和安全性
...
- final：修饰符，用于修饰类、变量和方法。修饰变量时，该变量使用中不被改变，必须在声明时给定初值，在引用中只能读取不可修改，即为常量。修饰方法时，说明方法不能在子类中被重写。修饰类时，说明这个类不可被继承。
...
- finalize：Object类中定义的方法，属于 Java 垃圾回收（GC）机制的一部分。Java中允许使用finalize() 方法在垃圾收集器将对象从内存中清除出去之前做必要的清理工作。这个方法是由垃圾收集器在销毁对象时调用的。
...
3. **toString()**：返回**对象的字符串表示**，子类重写后可输出特定格式的业务属性（如 User 类返回 “User {id=1, name=' 张三 '}”）；
4. **getClass()**：返回对象的运行时类（Class 对象），是反射的基础，且方法被 final 修饰，无法重写（如`user.getClass()`获取 User.class）。

 二、线程协作方法（用于多线程通信）

1. **wait()**：让**当前线程释放对象锁并进入等待状态**，需在**同步代码块 / 方法**中调用，可被 notify ()/notifyAll () 唤醒；
...
3. TreeMap：红黑树实现，key 自然排序， 实现 `Comparable` 接口并覆写 `compareTo()` 方法，TreeMap 会按该方法的返回值升序排列 Key。



13. #### ArrarList和LinkedList区别

- ArrayList是基于动态数组实现的数据结构，LinkedList基于链表的数据结构。
- 对于随机访问get和set，ArrayList效率优于LinkedList，因为LinkedList访问元素要移动指针，而arraylist可以通过下标实现随机访问。
...
- 数组：作为基础存储，保证**快速定位**，数组的核心优势是**通过索引随机访问的时间复杂度为 O (1)**，这是 HashMap 查询高效的基础
- 链表：解决哈希冲突，兼容少量冲突场景，当多个 key 哈希到同一个数组下标时（哈希冲突），链表用于串联这些冲突的键值对：

- 红黑树：优化长链表，降低查询复杂度，JDK8 引入红黑树，当链表长度超过阈值（默认 8）且数组容量≥64 时，链表转为红黑树，提升查询性能，从原来的O(n)到O(logn)


红黑树是一种自平衡的二叉搜索树，通过给每个节点标记红 / 黑颜色，保证树的高度始终维持在 O (log n) 级别

规则：根黑，叶黑，红节点子必黑，任意节点到叶节点的黑路径长度相同
...
- Hashtable：**线程安全**，所有核心方法（put/get/remove）都被`synchronized`修饰,也就是加上了全局锁，多线程下能**保证操作原子性**
...
2. **字符流**：以字符（按编码转换，如 UTF-8、GBK）为单位处理数据，仅适用于文本文件，避免字节流处理文本时的编码乱码问题；
   ...
   - 常用实现：`FileReader`/`FileWriter`（文件字符流）、
   - `BufferedReader`/`BufferedWriter`（缓冲字符流）、
   - `InputStreamReader`/`OutputStreamWriter`（转换流，字节流→字符流）。
...
反射是 Java 的核心特性之一，**允许程序在运行时（而非编译时）获取类的完整信息，并操作类 / 对象的属性、方法、构造器等成员**，无需提前知道类的具体名称，是实现Spring、MyBatis等框架的核心底层技术。

Java的反射机制的实现要借助于4个类：class，Constructor，Field，Method;通过这四个对象可以粗略的知道一个类的各个组成部分。
...
> 在运行时调用任意一个对象的方法
>
> getDeclaredField(String name)，getDeclaredMethod(String name, Class<?>... paramTypes)，获取本类**所有访问权限**的字段（private/protected/ 默认 /public，不含父类），方法

#### 核心特点

1. **打破封装**：能访问类的私有成员（私有属性、私有方法等），绕过编译期的访问限制,核心是通过`setAccessible(true)`来关闭访问检查；
...
反射的**第一步**是获取`Class`对象，共有 3 种常用方式，且三种方式获取的`Class`对象完全相同（内存中唯一）：
...
序列化（Serialization）是一种用来处理对象流的机制，指将 Java 对象的**属性值** 转换为**字节序列**的过程；对应的，反序列化是将字节序列恢复为原有对象的过程。

是为了实现对象的**持久化**（如存入文件、数据库）或**网络传输**（如分布式系统间的对象通信）因为对象仅存在于 JVM 内存中，而字节序列可存储 / 传输，反序列化后能还原出与原对象状态一致的新对象。

序 列 化 的 实 现 ： 将 需 要 被 序 列 化 的 类 实 现 Serializable 接 口 ， 该 接 口 没 有 需 要 实 现 的 方 法 ， implements Serializable 只是**为了标注该对象是可被序列化的**，然后使用一个输出流(如：FileOutputStream)来构造一个ObjectOutputStream(对象流)对象，就可以将对象写出，要恢复的话则用输入流。



- Java的跨平台特性

Java 的 “跨平台” 特性（即 “一次编写，到处运行”，Write Once, Run Anywhere）是其核心优势之一，其实现原理并非直接让 Java 代码与操作系统交互，而是通过**中间层（JVM）** 实现 “平台隔离”。对 Java 程序而言，JVM 提供了统一的运行环境，程序的编写不需依赖操作系统，JVM 分别适配了不同系统的 API（如文件操作、内存管理、线程调度），而对外提供统一的程序接口。所以实际上是JVM实现了对不同平台的兼容性
...
2. **核心作用**：区分 “属于类的资源” 和 “属于对象的资源”，静态变量是类的全局共享变量，所有实例共用一份内存，解决 “数据共享” 与 “内存优化” 问题（避免重复存储相同数据）。
...
ArrayList：随机访问效率更高O（1），遍历效率高（普通 for 循环），缓存友好（连续内存）；

增删需移动大量元素，效率低；扩容时复制数组，存在性能损耗；预留内存导致空间浪费，大数据量下内存占用更高。

 LinkedList：节点的增删操作（如迭代器 `remove()`）无需移动元素，效率高；非联系内存，无扩容开销，内存按需分配，空间利用率高；

随机访问性能极差，无法直接通过索引定位；普通 for 循环遍历效率极低，必须用迭代器；每个节点存储前后指针，内存开销比 ArrayList 大；
...
JUnit 是 Java 领域最主流的单元测试框架，基于注解（如 @Test 标记测试方法）实现自动化单元测试，一次性执行项目中所有测试方法，支持测试方法独立执行、断言验证等功能
...
动态代理是 Java 基于**反射**实现的设计模式，核心作用是：**动态为目标对象创建一个代理对象，通过这个代理对象间接调用目标方法，从而在调用前后插入额外逻辑（如日志、权限校验、事务控制），不需要修改目标类的源码**。
...
- **提高代码重用性**：将一些通用逻辑抽离到代理中，所有业务方法复用同一套代理逻辑，无需重复编写。
- **功能自动增强**：新增业务方法（如`select2()`）时，无需额外编写增强逻辑，代理会自动对新方法生效（只要新方法属于接口定义的方法）。
- **低侵入性**：无需修改被代理对象的代码，即可通过代理新增功能，符合软件设计的 “开闭原则”（对扩展开放，对修改关闭）。

**动态代理是 Spring AOP 的底层实现技术**，Spring 通过封装动态代理（JDK/CGLIB），实现了 AOP 的 “面向切面编程” 能力，让开发者无需手写代理逻辑，就能便捷地实现代码增强。





AOP是 Java 中补充 OOP（面向对象编程）的一种编程思想，核心是**将分散在业务代码中的通用横向逻辑（如日志、事务、权限校验）抽离出来，单独封装为 “切面”，再动态植入到业务方法的指定位置**，实现代码复用。
...
JVM（Java Virtual Machine，Java 虚拟机）是运行 Java 字节码的跨平台虚拟计算机，它屏蔽了不同操作系统（Windows、Linux、macOS）的底层差异，将编译后的.class 字节码文件解释 / 编译为对应系统的机器码执行，实现 “一次编写，到处运行” 的核心特性；同时 JVM 负责内存管理（堆 / 栈分配、垃圾回收）、线程调度、安全校验等核心能力，是 Java 程序从字节码到系统执行的中间层，也是 Java 跨平台、自动内存管理的关键基础。
...
| 分区名称       | 归属（共享 / 私有） | 核心作用                                                     | 关键特征                                                     |
| -------------- | ------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **程序计数器** | 线程私有            | 记录当前线程执行的字节码行号（如指令地址），线程切换时恢复执行位置 | 唯一不会 OOM 的区域；Native 方法时计数器值为 undefined       |
| **虚拟机栈**   | 线程私有            | 存储方法调用的栈帧（局部变量、方法返回地址、操作数栈等），即 “栈内存” | 方法调用入栈，执行完出栈；栈深度超限抛 StackOverflowError，扩容失败抛 OOM |
| **本地方法栈** | 线程私有            | 作用同虚拟机栈，但为 Native 方法（如 JNI 调用 C/C++ 方法）提供服务 | 与虚拟机栈逻辑一致，HotSpot 将其与虚拟机栈合并实现           |
| **堆（Heap）** | 线程共享            | 存储所有对象实例、数组，是 GC（垃圾回收）的核心区域，即 “堆内存” | JVM 启动时创建；可分为新生代（Eden+Survivor）、老年代，频繁 GC；溢出抛 OOM |
| **方法区**     | 线程共享            | 存储类信息、常量、静态变量、JIT 编译后的代码等（JDK8 后叫元空间） | JDK7 前用永久代实现，JDK8 后改用元空间（基于本地内存）；溢出抛 O |
...
程序实际上是在内存中的 JVM虚拟机中运行的，将内存分为三块：方法区、栈内存、堆内存。 通过**“三区域协作”**更好的执行程序：

| 内存区域 |            功能作用             |                         存储内容示例                         |
| :------: | :-----------------------------: | :----------------------------------------------------------: |
|  方法区  | 存放 .class文件，储存类模板信息 | `Student`类的定义（包含`name`、`chinese`、`math`属性的结构） |
